...
class mysql {
    ...
    /*
    * is_available:1-还在关注；0-不在关注
    */
    public function saveIntoUser($ToUserName,$FromUserName,$CreateTime,$MsgType,$Event,$BackMsg,$Identity=1,$BackMsgType="text"){
        $time = date('Y-m-j g:i:s');
        $this->query("insert into wx_user(ToUserName,FromUserName,CreateTime,MsgType,Event,BackMsg,BackMsgType,BackMsgTime,IsAvaible) values ('$ToUserName','$FromUserName','$CreateTime','$MsgType','$Event','$BackMsg','$BackMsgType','$time',1)");
        $this->updateUserMutual($FromUserName,$time,$MsgType);
    }

    public function updateUserMutual($FromUserName,$LastMutualTime,$LastMutualType){
        $this->query("update wx_user set MutualTimes = MutualTimes+1,LastMutualTime = '$LastMutualTime',LastMutualType = '$LastMutualType' where FromUserName = '$FromUserName' and IsAvaible = 1");
    }

    public function userUnStub($FromUserName,$MsgType){
        $time = date('Y-m-j g:i:s');
        $this->query("update wx_user set IsAvaible = 0 where FromUserName = '$FromUserName' ");
        $this->updateUserMutual($FromUserName,$time,$MsgType);
    }


    public function saveIntoText($ToUserName,$FromUserName,$CreateTime,$Content,$BackMsg,$MsgType,$MsgId,$BackMsgtype="text"){
        $time = date('Y-m-j g:i:s');
        $this->query("insert into wx_msg(ToUserName,BackMsgTime,FromUserName,CreateTime,Content,BackMsg,MsgType,BackMsgtype,MsgId)
         values ('$ToUserName','$time','$FromUserName','$CreateTime','$Content','$BackMsg','$MsgType','$BackMsgtype','$MsgId')");
        $this->updateUserMutual($FromUserName,$time,$MsgType);
    }

    public function saveIntoVoice($ToUserName,$FromUserName,$CreateTime,$MsgType,$MediaId,$Format,$Recognition,$MsgId,$BackMsg,$BackMsgType="text"){
        $time = date('Y-m-j g:i:s');
        $this->query("insert into wx_voice(ToUserName,FromUserName,CreateTime,MsgType,MediaId,Format,Recognition,MsgId,BackMsg,BackMsgTime,BackMsgType)
         values ('$ToUserName','$FromUserName','$CreateTime','$MsgType','$MediaId','$Format','$Recognition','$MsgId','$BackMsg','$time','$BackMsgType')");
        $this->updateUserMutual($FromUserName,$time,$MsgType);
    }
